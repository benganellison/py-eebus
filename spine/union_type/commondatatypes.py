# Jinja Template message_type.py.jinja2
from __future__ import annotations
from typing import TYPE_CHECKING
from spine.base import SpineBase, spine_type
from spine.type_registry import TypeRegistry

if TYPE_CHECKING:
    pass



@spine_type('ns_p:FunctionType', is_value_type=True, no_attrib_name=False)
class FunctionType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:FunctionType -> UnionType
    _MEMBER_INFO = [
        {
            "name": "value",
            "xml_name": "value",
            "type": "xs:string",
            "is_array": False,
            "is_optional": True,
            "class_check": "str"
        },
    ]


@spine_type('ns_p:FeatureSpecificUsageType', is_value_type=True, no_attrib_name=False)
class FeatureSpecificUsageType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:FeatureSpecificUsageType -> UnionType
    _MEMBER_INFO = [
        {
            "name": "value",
            "xml_name": "value",
            "type": "xs:string",
            "is_array": False,
            "is_optional": True,
            "class_check": "str"
        },
    ]


@spine_type('ns_p:FeatureSpecificUsageEnumType', is_value_type=False, no_attrib_name=False)
class FeatureSpecificUsageEnumType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:FeatureSpecificUsageEnumType -> UnionType
    _MEMBER_INFO = [
    ]


@spine_type('ns_p:FeatureTypeType', is_value_type=True, no_attrib_name=False)
class FeatureTypeType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:FeatureTypeType -> UnionType
    _MEMBER_INFO = [
        {
            "name": "value",
            "xml_name": "value",
            "type": "xs:string",
            "is_array": False,
            "is_optional": True,
            "class_check": "str"
        },
    ]


@spine_type('ns_p:EntityTypeType', is_value_type=True, no_attrib_name=False)
class EntityTypeType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:EntityTypeType -> UnionType
    _MEMBER_INFO = [
        {
            "name": "value",
            "xml_name": "value",
            "type": "xs:string",
            "is_array": False,
            "is_optional": True,
            "class_check": "str"
        },
    ]


@spine_type('ns_p:DeviceTypeType', is_value_type=True, no_attrib_name=False)
class DeviceTypeType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:DeviceTypeType -> UnionType
    _MEMBER_INFO = [
        {
            "name": "value",
            "xml_name": "value",
            "type": "xs:string",
            "is_array": False,
            "is_optional": True,
            "class_check": "str"
        },
    ]


@spine_type('ns_p:ScopeTypeType', is_value_type=True, no_attrib_name=False)
class ScopeTypeType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:ScopeTypeType -> UnionType
    _MEMBER_INFO = [
        {
            "name": "value",
            "xml_name": "value",
            "type": "xs:string",
            "is_array": False,
            "is_optional": True,
            "class_check": "str"
        },
    ]


@spine_type('ns_p:CurrencyType', is_value_type=False, no_attrib_name=False)
class CurrencyType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:CurrencyType -> UnionType
    _MEMBER_INFO = [
        {
            "name": "value",
            "xml_name": "value",
            "type": "xs:string",
            "is_array": False,
            "is_optional": False,
            "class_check": "str"
        },
    ]


@spine_type('ns_p:EnergyModeType', is_value_type=False, no_attrib_name=False)
class EnergyModeType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:EnergyModeType -> UnionType
    _MEMBER_INFO = [
    ]


@spine_type('ns_p:EnergyDirectionType', is_value_type=False, no_attrib_name=False)
class EnergyDirectionType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:EnergyDirectionType -> UnionType
    _MEMBER_INFO = [
    ]


@spine_type('ns_p:CommodityTypeType', is_value_type=True, no_attrib_name=False)
class CommodityTypeType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:CommodityTypeType -> UnionType
    _MEMBER_INFO = [
        {
            "name": "value",
            "xml_name": "value",
            "type": "xs:string",
            "is_array": False,
            "is_optional": True,
            "class_check": "str"
        },
    ]


@spine_type('ns_p:OccurrenceType', is_value_type=False, no_attrib_name=False)
class OccurrenceType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:OccurrenceType -> UnionType
    _MEMBER_INFO = [
    ]


@spine_type('ns_p:RecurringIntervalType', is_value_type=False, no_attrib_name=False)
class RecurringIntervalType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:RecurringIntervalType -> UnionType
    _MEMBER_INFO = [
    ]


@spine_type('ns_p:AbsoluteOrRelativeTimeType', is_value_type=False, no_attrib_name=False)
class AbsoluteOrRelativeTimeType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:AbsoluteOrRelativeTimeType -> UnionType
    _MEMBER_INFO = [
    ]


@spine_type('ns_p:UnitOfMeasurementType', is_value_type=True, no_attrib_name=False)
class UnitOfMeasurementType(SpineBase): # EEBus_SPINE_TS_CommonDataTypes.xsd:ns_p:UnitOfMeasurementType -> UnionType
    _MEMBER_INFO = [
        {
            "name": "value",
            "xml_name": "value",
            "type": "xs:string",
            "is_array": False,
            "is_optional": True,
            "class_check": "str"
        },
    ]

